...
| Roadmap item               | `docs/roadmap/`                                      | Conditional                               | Durable target output, scope, non-goals, appetite, first usable slices, outcome-level sequencing, and validation targets                | Product requirements, behavior specs, live issue status, sub-issue inventories, PR lists, assignees, schedules, capability inventories, or execution history | Product direction is larger than one PR and needs durable framing                                              |
...
translate stable-enough product intent into the next outcome path: appetite,
first usable slices, pilots, sequencing, validation targets, and ownership
boundaries.

Roadmaps are adjacent to product requirements but do not replace them. Product
requirements own the product problem, users, goals, outcomes, broad
requirements, assumptions, risks, and open questions. Roadmaps own the durable
path from that intent to target outputs and validation signals.
...
contain live issue status, sub-issue inventories, PR lists, assignees,
schedules, agent run state, single-PR implementation plans, capability
inventories that duplicate source files or navigation maps, validation summary
stores, postmortem archives, execution ledgers, or other history that belongs
in trackers, PRs, CI/check systems, source tests, Git history, or linked
evidence.
