...
flowchart TD
  ...
  candidate --> map
  map --> govern["AFDS workflow capability governance"]
  govern --> result["Classification result, blocker, or follow-up"]
  result --> approval["Separate approved issue before source behavior changes"]
```
...
procedure; the
[AFDS Workflow Capability Governance](afds-workflow-capability-governance.md)
guideline owns classification decisions and this map does not approve new
reusable assets by itself.
...
| Concrete user-confirmed proposal                       | Confirmed behavior change has no feature spec yet.    | Is the confirmed context sufficient to draft proposed work? | [Routing spec SLICE-001 and EVID-005](../specs/afds-workflow-routing.md#slice-001-issue-draft-origins).              | Attributed user-confirmed context plus available current source, test, or documentation evidence. | Confirm behavior, scope, boundaries, acceptance criteria, verification expectations, current evidence, and documentation owner or destination. Draft one provider-neutral issue body; distinguish current, proposed, and unresolved decisions, and include future documentation as pending scope. | Draft-only provider-neutral issue body before feature-spec authoring.                          | Treating a future document as accepted evidence; implementation, tracker publication, or bypassing existing-spec readiness. | `Blocked: proposal evidence or an execution-critical decision is missing.`     | Feature-spec authoring and readiness review when durable behavior documentation is needed; implementation only after separate authorization. |
...
Follow-up workflow surfaces are candidates for later
[AFDS Workflow Capability Governance](afds-workflow-capability-governance.md).
...
Do not classify the gap in this guideline. AFDS workflow capability governance
...
classifying AFDS workflow capabilities. The classification procedure is owned by
[AFDS Workflow Capability Governance](afds-workflow-capability-governance.md).
...
AFDS workflow capability governance remains responsible for deciding the
classification outcome for each candidate capability.
