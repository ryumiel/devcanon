...
## Documentation Abstraction Ceiling

This section is the sole complete operational rule for the boundary between
workflow documentation and executable mechanics. It allocates the boundary for
...
an operator enough policy to invoke the owner and interpret the result.
Executable helpers and runtime modules own deterministic algorithms, including
validation sequences, state transitions, environment normalization,
temporary-file identity, cleanup, retries, and failure classification.

### `SKILL.md` invocation contracts

When a skill delegates deterministic work, its `SKILL.md` identifies the
workflow decision and owner, the executable helper responsible, the helper's
required inputs and outputs, and how success or refusal affects continuation.
It does not restate the helper algorithm as shell-like prose. Keep the
...
interpretation, escalation, and continuation in `SKILL.md`. Every required
public script-backed action point must provide either the complete concrete
invocation for that action or the exact local `<script> --help` discovery step
for its executable; it retains workflow result and continuation judgment. Do
not duplicate authoritative invocation mechanics, input/output tables, refusal
or error details, or explanatory help prose between these owners. A minimal
command line at the owning workflow action point is permitted when needed to
execute the action; it does not become a second mechanics owner. Apply the generic hardening rule in
...
direct observable behavior, stable intentional public interfaces, executable
syntax or wire tokens, focused helper or runtime behavior, and concise
installed composition: presence, parseability, packaging, and canonical
references. Installed-composition checks are integration smoke coverage; they
must not duplicate a helper or runtime failure matrix.

Do not add proof-for-proof coverage for exact narrative wording or ordering,
explanatory-phrase uniqueness, source-string counts, documentation mutation
matrices, reviewer ceremony, or fixture-only matrices without an authoritative
consumer failure. Exact text remains appropriate only when an authoritative
consumer intentionally treats it as executable syntax, a wire token, or a
stable public interface.

### Review and mutation routing

Review remains broad and adversarial; this boundary does not limit discovery.
The [Code Review Guideline](code-review-guideline.md) remains authoritative for
review semantics. Before mutating source in response to a finding, classify it
as one of:
...
This allocation applies the rationale in
[ADR-0019](../adr/adr-0019-script-authority-for-deterministic-skill-mechanics.md),
...
Start with the observable failure and its authoritative consumer. Name the
owner, consumer, failure mode, and smallest assertion before adding a test.
Choose one of these coverage routes:

- **Structural source contract:** Parse the authoritative Markdown structure
  when a table, heading, fenced block, or link registry is intentionally
  machine-readable. Assert the parsed fields or relationships, not nearby
  narrative fragments.
- **Render or packaging contract:** Exercise the renderer when target metadata,
  sidecars, placeholder expansion, or mirrored supporting files are the
  behavior being protected. Derive coverage inventories from source structure.
- **Executable runtime or artifact contract:** Run the owning helper, runtime,
...
counts, or duplicated explanations merely because an agent reads the prose.

When a change introduces or materially changes generated artifacts, helper I/O
files, `.ephemeral` handoffs, cross-skill handoffs, or other side-channel data,
apply the Side-Channel Artifact Contract Checklist in
`docs/guidelines/documentation-checklists.md` before choosing a route.

Generated output is disposable verification evidence. Source files own the
contract; source tests and render tests verify it. Do not make `generated/`
files authoritative, and do not snapshot every generated byte for incidental
prose changes that have no generated-output contract.
...
- [ ] Structural, render/packaging, executable/runtime, behavioral-evaluation,
      or editorial coverage chosen according to § 8.
