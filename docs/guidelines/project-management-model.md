...
1. **Repository owns durable truth**: specs, architecture, contracts, and decisions belong in repo docs and code. Example: SPEC.md defines config format -- not a GitHub issue comment.
...
| -------------------- | ------------------------------------------------------ | ------------------------------ | -------------------------------------------------------- |
| **Spec**             | Durable statement of intended behavior and constraints | `SPEC.md`                      | "Config file must be named `agents-manager.config.yaml`" |
...
| --------------------------------------------- | ----------------- | ---------------------- |
| Intended behavior, constraints, format specs  | SPEC.md           | Issue comments         |
| Repository structure, module responsibilities | AGENTS.md, MAP.md | Agent session notes    |
...
- If resolution creates durable decisions or constraints not obvious from code, capture in the owning doc (SPEC.md, CONTRIBUTING.md, etc.) in the same PR.
...
- Config format or schema -> SPEC.md
