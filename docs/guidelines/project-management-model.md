...
1. **Repository owns durable truth**: specs, architecture, contracts,
   guidelines, roadmap direction, and decisions belong in repo docs and code.
   Example: `docs/specs/configuration.md` defines config format, not an issue
   comment.
...
reusable procedure, contract ownership, or roadmap-scale intent.

The shape path is:

```text
raw idea -> durable spec/guideline/roadmap/ADR artifact -> issue slicing
```

Shape-path work produces or updates the durable artifact first. Issues then
slice implementation work from that artifact.
...
These variants do not require a new product spec unless the current uncertainty
is product/domain behavior, workflow policy, reusable procedure, contract
ownership, or roadmap-scale intent.

## 5. Issue Types

### 5.1 Spec-writing issue

A spec-writing issue exists to shape durable behavior or policy. It should
identify the owning artifact, the uncertainty to resolve, and any implementation
issues expected after the durable artifact lands.

### 5.2 Implementation issue

An implementation issue is already sliced enough to execute. It may reference an
owning spec, but it can also use repro steps, tests, audit output, PR comments,
existing code investigation, stable requirement IDs, scenario IDs, headings, or
explicit acceptance criteria as the immediate execution contract.
...
### 5.5 Skill/agent issue

A skill/agent issue changes reusable workflow behavior, role wrappers, prompt
...
- If resolution creates durable decisions or constraints not obvious from code,
  capture them in the owning doc (`docs/specs/`, `docs/adr/`,
  `docs/guidelines/`, etc.) in the same PR.
