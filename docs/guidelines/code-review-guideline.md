...
| **P1**   | Architectural boundaries | Yes       | Render module (`src/render/`) must not depend on install module (`src/install/`) or vice versa. Config schema changes in `src/config/schema.ts` must stay aligned with SPEC.md § 9-11. Target-specific logic (Claude vs Codex) must stay inside respective renderers, not leak into shared code. Skill and agent models must match the source schema defined in SPEC.md. |
