...
The Scope rule — including the positive frame, the categories-vs-identity boundary, and the negative list of items that must never appear in an issue body or title — is canonically defined in [`skills/report-agents-manager-shared-issue/SKILL.md` § Scope: What to Describe](../../skills/report-agents-manager-shared-issue/SKILL.md#scope-what-to-describe). Apply that rule when filling any body field below.
...
3. Apply the § 2 Scope rule to the body. For any quoted material, follow the Redaction Gate in [`skills/report-agents-manager-shared-issue/SKILL.md` § Redaction Gate](../../skills/report-agents-manager-shared-issue/SKILL.md#redaction-gate). The redaction rules are canonical there.
...
For title format and `(scope)` rules, see [`../../skills/report-agents-manager-shared-issue/SKILL.md` § Issue Draft Shape](../../skills/report-agents-manager-shared-issue/SKILL.md#issue-draft-shape).

When filling WORKFLOW.md's "Environment and provenance" body field for a shared-skill or shared-agent report, include target (`codex`, `claude`, or both), install mode (`symlink`, `copy`, or `unknown`), `agents-manager` revision/branch/version, and whether the problem still reproduces after `render` and after `sync`. These fields are gathered per § 3 step 2.
...
<!-- Canonical home for the retest loop. skills/report-agents-manager-shared-issue/SKILL.md § Upstream Fix Loop mirrors these steps in spirit. Update both together when the loop changes. -->
