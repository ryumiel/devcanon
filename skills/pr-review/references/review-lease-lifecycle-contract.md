...
| LC-24 | `recover-execution-receipt`        | `failed`              | `resolving` | One of the two closed non-definitive tuples defined below preserves a stored intent and no receipt; valid receipt with a pending or failed sealed resolve, `GITHUB_POSTED_AT`, `PROVIDER_REVIEW_ID`, `UPDATED_AT`                           |
| LC-25 | `recover-complete-receipt`         | `failed`              | `posted`    | One of the two closed non-definitive tuples defined below preserves a stored intent and no receipt; valid terminal receipt, `GITHUB_POSTED_AT`, `PROVIDER_REVIEW_ID`, `FINISHED_AT`, `UPDATED_AT`                                           |
...
the intended replacement; any other bytes fail closed. The only valid
non-definitive tuples are `phase=github-post`, `recoverability=unknown`, and
either reason `GitHub review POST outcome is uncertain` for transport or 5xx
failure, or reason `GitHub review POST outcome is indeterminate` for an
unvalidated success or failed exact-match reconciliation. Both require the
stored intent, no receipt, `github_post_attempted=true`,
`github_post_result=failed`, and null posted time and provider review ID. Either
tuple grants no resolution, repost, cleanup, archive, or reentry authority. The exact definitive
...
Either closed non-definitive tuple above may proceed only through LC-24 or
LC-25 after provider
...
no retry, reconciliation, provider access, or old authority reuse. The two
closed non-definitive tuples remain incomplete regardless of override.
