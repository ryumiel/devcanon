...
| LC-04 | `present-preview`             | `reviewed`            | `gated`    | Existing bound result pointer, `PRESENTED_AT`, `PRESENTATION_STATUS`, `UPDATED_AT`; optional `RESULT_FILE` must validate and bind when supplied                                                                                                           | Bind result with `preview-current` result-manifest status; set fresh presentation; clear approved-review, terminal, failure, and GitHub metadata.                 |
| LC-05 | `present-preview`             | `gated`               | `gated`    | Existing bound result pointer, `PRESENTED_AT`, `PRESENTATION_STATUS`, `UPDATED_AT`; optional `RESULT_FILE` must validate and bind when supplied                                                                                                           | Require materially fresh presentation data; bind current result; replace presentation; clear approved-review, terminal, failure, and GitHub metadata.             |
...
| LC-14 | `present-preview`             | `failed`              | `gated`    | Existing bound result pointer, `PRESENTED_AT`, `PRESENTATION_STATUS`, `UPDATED_AT`; optional `RESULT_FILE` must validate and bind when supplied                                                                                                           | Recover through a fresh preview; bind result with `preview-current`; replace presentation; clear approved-review, terminal, failure, and GitHub metadata.         |
...
| LC-16 | `record-failure`              | `failed`              | `failed`   | `FINISHED_AT`, `FAILURE_PHASE`, `FAILURE_REASON`, `FAILURE_RECOVERABILITY`, `UPDATED_AT`; when `FAILURE_PHASE=github-post`, prior failure must be a GitHub-post failure and inputs must include `GITHUB_POST_ATTEMPTED=true`, `GITHUB_POST_RESULT=failed` | Require materially new failure audit data; preserve recovery artifacts allowed by the new phase; clear phase-inapplicable GitHub metadata.                        |
...
adapters record cleanup metadata and perform plain or forced
`git worktree remove` only after classification.
...
classifier decision; it must not duplicate classification logic. When removal
would record matching lease metadata, the metadata update must be writable before
`git worktree remove` runs so metadata write failures do not remove the review
worktree first.
