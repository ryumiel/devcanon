...
`write-result` requires `PR_NUMBER`, `HEAD_SHA`, `REPOSITORY`, `FINDINGS_FILE`, `SCOPE_DECISION_FILE`, and `PRESENTATION_STATUS`; `PRIOR_THREADS_FILE`, `REVIEW_BODY_FILE`, `CONTEXT_FILE`, `RENDERED_PREVIEW_FILE`, and `PRESENTATION_NOTES` are optional. `validate-result`, `read-result-for-preview`, `write-review-body`, and `recover-review-body-publication` require `REPOSITORY` and `RESULT_FILE`; body writing reads Markdown from stdin, while the other three do not. `replace-findings` requires `PR_NUMBER`, `HEAD_SHA`, `REPOSITORY`, `RESULT_FILE`, and `PLAY_REVIEW_HELPER`, reads exactly one complete findings envelope from stdin, and accepts no extra argument. `render-phase5-audit-summary` requires `REPOSITORY`, `PR_NUMBER`, `HEAD_SHA`, `RESULT_FILE`, `PRIMARY_REPOSITORY_ROOT`, `WORKTREE_PATH`, and `LEASE_FILE`, and reads no stdin.
