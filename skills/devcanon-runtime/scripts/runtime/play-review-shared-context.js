...
export async function runPlayReviewSharedContextCommand(args) {
    try {
        ...
        switch (commandName) {
            ...
            default:
                throw new SharedContextError("usage: shared-review-context.sh write-review-context-input|build-review-context");
        }
    }
    catch (err) {
...
async function writeReviewContextInput() {
    const repoRoot = await requireRepoRoot();
    ...
    const manifest = parseManifestPayload(requiredEnv("REVIEW_CONTEXT_INPUT_JSON"));
    validateManifestBindings(manifest, headSha, repoRoot);
    await prepareManifestWriteTarget(inputFile);
    await writeTextUnderEphemeral(inputFile, `${JSON.stringify(manifest, null, 2)}\n`);
    await guardManifestReadTarget(inputFile);
    return inputFile;
}
async function buildReviewContext() {
    ...
    validateFindingsPath(findingsFile, headSha);
    const { inputFile: expectedInputFile, outputFile: reviewContextOutputFile } = deriveContextPaths(findingsFile);
    validateDirectEphemeralPath("review context input", reviewContextInputFile, "-review-context-input.json");
    if (reviewContextInputFile !== expectedInputFile) {
        throw new SharedContextError(`review context input path mismatch: ${reviewContextInputFile}`);
    }
...
async function requireRepoRoot() {
    ...
    if (physicalToplevel !== physicalCwd) {
        throw new SharedContextError("shared-review-context.sh must run from the repository root");
    }
...
function validateHeadSha(headSha) {
    if (!/^[0-9a-f]{40}$/u.test(headSha)) {
        throw new SharedContextError("HEAD_SHA must be a 40-character lowercase hex SHA");
    }
}
function validateDirectEphemeralPath(label, file, suffix) {
    if (file.includes("..")) {
        throw new SharedContextError(`path traversal: ${file}`);
    }
    if (file.startsWith("/")) {
        throw new SharedContextError(`${label} path must be repo-relative: ${file}`);
    }
...
function validateFindingsPath(findingsFile, headSha) {
    validateDirectEphemeralPath("findings", findingsFile, "-findings.json");
    if (!findingsFile.endsWith(`-${headSha}-findings.json`)) {
        throw new SharedContextError(`findings path must include HEAD_SHA: ${findingsFile}`);
    }
}
function deriveContextPaths(findingsFile) {
    const inputFile = findingsFile.replace(/-findings\.json$/u, "-review-context-input.json");
    const outputFile = findingsFile.replace(/-findings\.json$/u, "-review-context.md");
    validateDirectEphemeralPath("review context input", inputFile, "-review-context-input.json");
    validateDirectEphemeralPath("review context output", outputFile, "-review-context.md");
    return { inputFile, outputFile };
}
async function guardReadInputAndOutput(reviewContextInputFile, reviewContextOutputFile) {
    await guardEphemeralDirectory();
    await guardManifestReadTarget(reviewContextInputFile);
    const outputStat = await lstat(reviewContextOutputFile).catch(() => null);
    if (outputStat?.isSymbolicLink()) {
        throw new SharedContextError(`review context output must not be a symlink: ${reviewContextOutputFile}`);
    }
    if (outputStat?.isDirectory()) {
        throw new SharedContextError(`review context output path is a directory: ${reviewContextOutputFile}`);
    }
    if (outputStat !== null && !outputStat.isFile()) {
        throw new SharedContextError(`review context output exists but is not a regular file: ${reviewContextOutputFile}`);
    }
}
async function guardEphemeralDirectory() {
    const ephemeralStat = await lstat(".ephemeral").catch(() => null);
    if (ephemeralStat?.isSymbolicLink()) {
        throw new SharedContextError(".ephemeral must be a directory, not a symlink");
    }
    if (ephemeralStat !== null && !ephemeralStat.isDirectory()) {
        throw new SharedContextError(".ephemeral must be a directory");
    }
}
async function prepareManifestWriteTarget(inputFile) {
    await guardEphemeralDirectory();
    await mkdir(".ephemeral", { recursive: true });
    const inputStat = await lstat(inputFile).catch(() => null);
    if (inputStat?.isSymbolicLink()) {
        throw new SharedContextError(`review context input must not be a symlink: ${inputFile}`);
    }
    if (inputStat?.isDirectory()) {
        throw new SharedContextError(`review context input path is a directory: ${inputFile}`);
    }
    if (inputStat !== null && !inputStat.isFile()) {
        throw new SharedContextError(`review context input exists but is not a regular file: ${inputFile}`);
    }
}
async function guardManifestReadTarget(inputFile) {
    const inputStat = await lstat(inputFile).catch(() => null);
    if (inputStat?.isSymbolicLink()) {
        throw new SharedContextError(`review context input must not be a symlink: ${inputFile}`);
    }
    if (inputStat === null || !inputStat.isFile()) {
        throw new SharedContextError(`review context input missing or not a regular file: ${inputFile}`);
    }
...
async function readManifest(file) {
    ...
    try {
        ...
        throw new SharedContextError(`review context input missing or unreadable: ${file}`);
    }
    return parseManifestPayload(raw, file);
}
function parseManifestPayload(payload, fileForError = "<payload>") {
    let parsed;
    try {
    ...
    if (!isSharedContextInput(parsed)) {
        if (hasInvalidMode(parsed)) {
            throw new SharedContextError("manifest mode must be present, fix, or github-post");
        }
...
function validateManifestBindings(manifest, headSha, repoRoot) {
    if (manifest.header.head_sha !== headSha) {
        throw new SharedContextError(`manifest head_sha mismatch: ${manifest.header.head_sha}`);
    }
    if (manifest.header.working_directory !== repoRoot) {
        throw new SharedContextError(`manifest working_directory mismatch: ${manifest.header.working_directory}`);
    }
}
function buildCoreSection(manifest, headSha, findingsFile, reviewContextInputFile, repoRoot) {
    ...
    lines.add(`- **Base ref:** ${escapeUntrustedManifestText(manifest.header.base_ref)}`);
    lines.add(`- **Active diff range:** ${escapeUntrustedManifestText(manifest.header.active_diff_range)}`);
    lines.add(`- **Full PR diff range:** ${escapeUntrustedManifestText(manifest.header.full_pr_diff_range)}`);
    lines.add(`- **Mode:** ${manifest.header.mode}`);
    lines.add(`- **Language hints:** ${manifest.header.language_hints.map(escapeUntrustedManifestText).join(", ")}`);
    lines.add(`- **Changed-files command:** ${escapeUntrustedManifestText(manifest.changed_files.command)}`);
    lines.add(`- **Changed-files total:** ${manifest.changed_files.total_count}`);
    lines.add(`- **Changed-files truncated:** ${manifest.changed_files.truncated}`);
    lines.add("");
    ...
    if (manifest.changed_files.records.length === 0) {
        ...
        for (const record of manifest.changed_files.records) {
            lines.add(`- ${escapeUntrustedManifestText(record.status)} ${escapeUntrustedManifestText(record.path)}`);
        }
    }
    lines.add("");
    lines.add("### Documentation Impact");
    appendArrayValues(lines, "Architecture files", manifest.doc_impact_summary.arch_files);
    appendArrayValues(lines, "New ADRs", manifest.doc_impact_summary.new_adrs);
    appendArrayValues(lines, "Modified ADRs", manifest.doc_impact_summary.modified_adrs);
    appendRoutingRiskValues(lines, "Architecture routing risks", manifest.doc_impact_summary.architecture_routing_risks);
    appendRoutingRiskValues(lines, "Spec routing risks", manifest.doc_impact_summary.spec_routing_risks);
    if (manifest.doc_impact_summary.notes) {
        lines.add(`- **Notes:** ${escapeUntrustedMarkdownText(manifest.doc_impact_summary.notes)}`);
    }
    lines.add("");
    ...
    if (manifest.adr_references.length === 0) {
        ...
        for (const record of manifest.adr_references) {
            lines.add(`- ${escapeUntrustedManifestText(record.path)} - ${escapeUntrustedManifestText(record.reason)}`);
        }
    }
    lines.add("");
...
function appendArrayValues(lines, title, values) {
    lines.add(`- **${title}:**`);
    if (values.length === 0) {
        lines.add("  - (none)");
        return;
...
function appendRoutingRiskValues(lines, title, risk) {
    lines.add(`- **${title}:**`);
...
function buildGuidelineSection(manifest) {
    ...
    const blocks = records.map((record, index) => {
        ...
        const priorityDisplay = escapeUntrustedGuidelineText(record.priority ?? "unspecified");
        ...
        if (index >= GUIDELINE_ITEM_LIMIT) {
            ...
            baseLines.add(`- **Overflow:** record beyond ${GUIDELINE_ITEM_LIMIT} guideline item limit`);
            baseLines.add(`- Targeted reread: open ${pathDisplay} before relying on this guideline.`);
            baseLines.add("");
        ...
        baseLines.add(`- **Summary:** ${summaryDisplay}`);
        baseLines.add(`- Targeted reread: open ${pathDisplay} if this summary affects a finding.`);
        ...
        baseLines.add("- **Exact excerpt:** Exact excerpt omitted due to byte budget.");
        baseLines.add("");
        ...
        includedLines.add(`- Targeted reread: open ${pathDisplay} if this summary affects a finding.`);
        includedLines.add(`- **Exact excerpt bytes:** ${excerptBytes}`);
        includedLines.add(`- Exact excerpt: ${escapeUntrustedGuidelineText(excerpt)}`);
        includedLines.add("");
        return {
    ...
    return packRequiredFirstSection(header, blocks, GUIDELINE_BUDGET, "guideline section byte budget exceeded");
}
function buildPriorSection(manifest) {
    ...
    const blocks = records.map((record, index) => {
        ...
        const sourceReferenceDisplay = escapeUntrustedPriorText(record.source.reference);
        ...
        if (index >= PRIOR_ITEM_LIMIT) {
            ...
            baseLines.add(`- **Overflow:** record beyond ${PRIOR_ITEM_LIMIT} prior-review item limit`);
            baseLines.add(`- Targeted reread: inspect ${sourceReferenceDisplay} before relying on this prior review context.`);
            baseLines.add("");
            return { baseText: baseLines.toString() };
        }
        ...
        baseLines.add(`- **Summary:** ${summaryDisplay}`);
        baseLines.add("- Untrusted prior-review evidence: true");
        baseLines.add(`- Targeted reread: inspect ${sourceReferenceDisplay} if this untrusted summary affects a finding.`);
        ...
        includedLines.add(`- Targeted reread: inspect ${sourceReferenceDisplay} if this untrusted summary affects a finding.`);
        includedLines.add(`- **Exact excerpt bytes:** ${excerptBytes}`);
    ...
    return packRequiredFirstSection(header, blocks, PRIOR_BUDGET, "prior review section byte budget exceeded");
}
function packRequiredFirstSection(header, blocks, budget, errorMessage) {
    ...
    const suffixBaseBytes = new Array(blocks.length + 1).fill(0);
...
async function writeReviewContext(reviewContextOutputFile, content) {
...
async function writeTextUnderEphemeral(targetFile, content) {
...
function isSharedContextInput(value) {
    if (!isObject(value)) {
        return false;
    }
    ...
    return (value.schema === "play-review/shared-context-input/v1" &&
        isObject(header) &&
...
function changedFileCountIsConsistent(changedFiles) {
    if (!nonnegativeInteger(changedFiles.total_count) ||
        typeof changedFiles.truncated !== "boolean" ||
        !Array.isArray(changedFiles.records)) {
        return false;
    }
...
function isAdrReference(value) {
    return (isObject(value) &&
        repoRelativePath(value.path) &&
        nonemptyString(value.reason));
}
function isGuidelineRecord(value) {
    return (isObject(value) &&
        repoRelativePath(value.path) &&
...
function isPriorReviewRecord(value) {
    return (isObject(value) &&
...
function hasInvalidMode(value) {
    return (isObject(value) &&
        isObject(value.header) &&
        !isReviewMode(value.header.mode));
}
function hasInvalidGuidelineSummary(value) {
    if (!isObject(value) ||
        !isObject(value.discovered_guidelines) ||
        !Array.isArray(value.discovered_guidelines.records)) {
        return false;
    }
    return value.discovered_guidelines.records.some((record) => isObject(record) &&
        (typeof record.summary !== "string" || record.summary.length === 0));
}
function hasInvalidPriorUntrustedFlag(value) {
    const records = priorRecords(value);
    return records.some((record) => isObject(record) && record.untrusted !== true);
}
function hasInvalidPriorSummary(value) {
    const records = priorRecords(value);
    return records.some((record) => isObject(record) &&
        (typeof record.summary !== "string" || record.summary.length === 0));
}
function priorRecords(value) {
    if (!isObject(value) ||
        !isObject(value.prior_review_context) ||
        !Array.isArray(value.prior_review_context.records)) {
        return [];
...
function escapeUntrustedMarkdownText(text) {
    return JSON.stringify(JSON.stringify(text).slice(1, -1));
}
function renderJsonStringLiteral(text) {
    return JSON.stringify(text);
}
function escapeUntrustedPriorText(text) {
    return escapeUntrustedMarkdownText(text);
}
function escapeUntrustedGuidelineText(text) {
    return escapeUntrustedMarkdownText(text);
}
function escapeUntrustedManifestText(text) {
    return escapeUntrustedMarkdownText(text);
}
