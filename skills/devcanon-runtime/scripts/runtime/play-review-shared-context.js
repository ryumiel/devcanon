...
function buildGuidelineSection(manifest) {
    ...
    records.forEach((record, index) => {
        ...
        if (index >= GUIDELINE_ITEM_LIMIT) {
            ...
            lines.add(`- **Overflow:** record beyond ${GUIDELINE_ITEM_LIMIT} guideline item limit`);
            lines.add(`- Targeted reread: open ${pathDisplay} before relying on this guideline.`);
            lines.add("");
        }
        else {
            ...
            lines.add(`- **Summary:** ${summaryDisplay}`);
            lines.add(`- Targeted reread: open ${pathDisplay} if this summary affects a finding.`);
            ...
            if (excerpt === undefined) {
                ...
                if (excerptBytes <= GUIDELINE_EXCERPT_LIMIT &&
                    ...
                    lines.add("- **Overflow:** exact excerpt omitted due to byte budget.");
                    lines.add("- **Exact excerpt:** Exact excerpt omitted due to byte budget.");
                }
            }
            lines.add("");
        }
...
function buildPriorSection(manifest) {
    ...
    records.forEach((record, index) => {
        ...
        if (index >= PRIOR_ITEM_LIMIT) {
            ...
            lines.add(`- **Overflow:** record beyond ${PRIOR_ITEM_LIMIT} prior-review item limit`);
            lines.add(`- Targeted reread: inspect ${sourceReferenceDisplay} before relying on this prior review context.`);
            lines.add("");
        }
        else {
            ...
            lines.add(`- **Summary:** ${summaryDisplay}`);
            lines.add("- Untrusted prior-review evidence: true");
            lines.add(`- Targeted reread: inspect ${sourceReferenceDisplay} if this untrusted summary affects a finding.`);
