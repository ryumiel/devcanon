...
import { access, link, lstat, mkdtemp, readFile, readdir, realpath, rm, stat, writeFile, } from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { TextDecoder } from "node:util";
import { writeTextAtomically } from "./artifacts.js";
import { gitPathPairKey, isRepoPathIdentity, parseGitChangedFilesZ, parseGitNameStatusZ, parseGitNumstatZ, } from "./git-diff-parser.js";
import { providerBoundGitArgs, providerBoundGitEnv, runGit, runGitRaw, runGitStatus, runGitStdoutSha256, } from "./git.js";
...
export async function runPrReviewProviderScopeEvidenceCommand(args, operations = {}) {
    try {
        if (args[0] === "contract") {
            ...
            return ok(`${JSON.stringify(PR_REVIEW_PROVIDER_SCOPE_EVIDENCE_CONTRACT)}\n`);
        }
        if (args[0] === "materialize-capture") {
            return await materializeProviderScopeCapture(args.slice(1));
        }
        if (args[0] !== "write") {
            fail("usage: pr-review-provider-scope-evidence contract|materialize-capture|write --head-sha <sha> --capture-file <path>");
        }
        ...
        validateSuffix("--capture-file", captureFile, "-provider-scope-capture.json");
...
async function materializeProviderScopeCapture(args) {
    ...
    const providerFiles = pages.flat().map((value) => {
        ...
        return {
            ...
            previous_path: typeof file.previous_filename === "string"
...
async function providerScopeEvidenceFromCapture(capture, headSha) {
    ...
    const providerPrDiffBase = mergeBases[0];
    const range = `${providerPrDiffBase}..${headSha}`;
    await requireProviderBoundRangeExists(range);
    const captureFiles = normalizedCaptureEntries(arrayField(capture, "provider_files"));
    ...
    if (!jsonEqual(fileEntryMetadata(captureFiles), fileEntryMetadata(localFiles))) {
    ...
    const projectedLocalFiles = unavailableOnly
        ...
        : localFiles;
    if (!unavailableOnly) {
        validateProviderPatchEvidence(providerFiles, projectedLocalFiles, localFiles);
    }
    const providerDiff = objectField(capture, "provider_diff");
    const providerDiffBytes = strictBase64Bytes(stringField(providerDiff, "content_base64"), "provider diff bytes are invalid");
...
function unavailablePatchProjection(entry) {
...
async function validateScopeDecision(options) {
    ...
    const providerEvidence = options.surface === "pr-review"
    ...
    if (options.surface === "pr-review") {
        ...
        const providerDiffBase = stringField(providerEvidence, "provider_pr_diff_base_sha");
    ...
    const lastReviewedExists = lastReviewed.length > 0 &&
    ...
    if (isNarrow) {
        ...
        if (lastReviewed.length === 0) {
            ...
            await requireRangeExistsForSurface(options.surface, gitExecutionRange(candidateRange, options.headSha));
    ...
    const expectedFiles = options.surface === "pr-review"
    ...
    if (lastReviewed.length > 0 && followupUsable) {
        const countExecRange = gitExecutionRange(`${lastReviewed}..HEAD`, options.headSha);
        expectedCount =
            options.surface === "pr-review"
    ...
    if (lastReviewed.length > 0) {
        ...
        if (followupUsable) {
            ...
            const candidateExecRange = gitExecutionRange(expectedCandidateRange, options.headSha);
            const candidateFiles = options.surface === "pr-review"
...
async function validatePrReviewProviderEvidence(scope, options) {
    ...
    const evidenceDigest = createHash("sha256")
        .update(evidenceText)
        .digest("hex");
    if (evidenceDigest !== stringField(artifacts, "provider_scope_evidence_sha256")) {
        fail("provider scope evidence digest mismatch");
    }
    const evidence = await readSingleJsonObject(options.providerScopeEvidenceFile, "provider evidence JSON validation failed");
    validateProviderEvidenceShape(evidence);
    if (stringField(evidence, "baseRefOid").length > 0 &&
    ...
    const providerMergeBases = await providerBoundGitMergeBases(providerBase, providerHead);
    if (providerMergeBases.length !== 1 ||
    ...
    if (!jsonEqual(fileEntryMetadata(localFiles), fileEntryMetadata(expectedLocalFiles))) {
...
function isProviderFileEntry(entry) {
    ...
    return (hasExactKeys(file, [
        ...
        ["added", "modified", "removed", "renamed"].includes(stringField(file, "status")) &&
...
function validateProviderScopeCaptureShape(capture) {
...
function isProviderScopeCaptureDiff(value) {
    if (value === null || typeof value !== "object" || Array.isArray(value)) {
        return false;
    }
    const diff = value;
    return (hasExactKeys(diff, ["dialect", "content_base64"]) &&
        [CANONICAL_GIT_DIFF_DIALECT, GITHUB_PROVIDER_DIFF_DIALECT].includes(stringField(diff, "dialect")) &&
        isStrictBase64(stringField(diff, "content_base64")));
}
function isProviderScopeCaptureFileEntry(value) {
    if (value === null || typeof value !== "object" || Array.isArray(value)) {
        return false;
    }
    const entry = value;
...
function strictBase64Bytes(value, failure) {
    if (!isStrictBase64(value)) {
        fail(failure);
    }
    return Buffer.from(value, "base64");
}
function normalizedCaptureEntries(entries) {
    return entries
        .map((entry) => entry)
        .sort(compareCaptureFileEntries);
}
function compareCaptureFileEntries(left, right) {
    return captureFileEntryKey(left).localeCompare(captureFileEntryKey(right));
}
function captureFileEntryKey(entry) {
...
function validateNoDuplicateCaptureFileEntries(entries) {
    const seen = new Set();
...
function providerEvidenceEntry(captureEntry) {
    ...
    return {
        ...
        patch_sha256: patchBase64 === null
            ? null
            : createHash("sha256")
                .update(strictBase64Bytes(stringField(captureEntry, "patch_base64"), "provider patch bytes are invalid"))
                .digest("hex"),
        patch_available: patchBase64 !== null,
    };
}
function normalizedEvidenceEntries(entries) {
    return entries.map((entry) => entry).sort(compareFileEntries);
}
function fileEntryMetadata(entries) {
...
function validateProviderPatchEvidence(providerFiles, localFiles, expectedLocalFiles) {
    for (let index = 0; index < providerFiles.length; index += 1) {
        ...
        else if (!isUnavailablePatchEntry(providerFile) ||
...
async function normalizedLocalFileEntries(range) {
...
async function gitDiffNameStatus(range) {
    return parseGitNameStatusZ(await canonicalGitDiffMetadataRaw(["--name-status", "-z", range]));
}
async function canonicalGitDiffMetadataRaw(args) {
    return canonicalGitDiffRawWithArgs(args, { patch: false });
}
async function canonicalGitDiffRaw(range, pathspecs = []) {
    return canonicalGitDiffRawWithArgs([range, ...(pathspecs.length > 0 ? ["--", ...pathspecs] : [])], { patch: true, literalPathspecs: pathspecs.length > 0 });
}
async function canonicalGitDiffSha256(range, pathspecs = []) {
    return canonicalGitDiffSha256WithArgs([range, ...(pathspecs.length > 0 ? ["--", ...pathspecs] : [])], { patch: true, literalPathspecs: pathspecs.length > 0 });
}
async function canonicalGitDiffRawWithArgs(args, options) {
    return canonicalGitDiffOutputWithArgs(args, options, "raw");
}
async function canonicalGitDiffSha256WithArgs(args, options) {
    return canonicalGitDiffOutputWithArgs(args, options, "sha256");
}
async function canonicalGitDiffOutputWithArgs(args, options, output) {
    await assertProviderBoundGitPreflight();
...
async function providerBoundGit(args) {
...
async function providerBoundGitStatus(args) {
...
async function providerBoundGitRefExists(ref) {
    return (await providerBoundGitStatus(["cat-file", "-e", ref])) === 0;
}
async function providerBoundGitMergeBases(base, head) {
    try {
...
async function providerBoundGitMergeBaseIsAncestor(ancestor, descendant) {
    return ((await providerBoundGitStatus([
        "merge-base",
        "--is-ancestor",
        ancestor,
        descendant,
...
async function requireProviderBoundRangeExists(range) {
    if ((await providerBoundGitStatus(["diff", "--quiet", range])) > 1) {
        fail("review range does not resolve");
    }
}
async function assertProviderBoundGitPreflight() {
...
async function assertNoObjectGraphOverrides() {
...
async function assertNoLooseReplaceRefs(replaceRefsPath) {
    try {
...
async function assertNoPackedReplaceRefs(packedRefsPath) {
    try {
...
async function assertEmptyOrMissingObjectGraphFile(filePath) {
    try {
...
async function assertNoLocalDiffDriverConfig() {
    ...
    if ([...localDiffConfig, ...worktreeDiffConfig].some(isUnsafeDiffConfigEntry)) {
...
async function gitDiffNumstats(range) {
    ...
    for (const entry of parseGitNumstatZ(await canonicalGitDiffMetadataRaw(["--numstat", "-z", range]))) {
...
function rejectRiskSignalsExtraFlags(options) {
...
function isTrueBlockingFinding(finding) {
    return (stringField(finding, "severity") === "Blocking" &&
...
function validatePrReviewScopeArtifactsShape(scope) {
    const artifacts = objectField(scope, "artifacts");
    if (!hasExactKeys(artifacts, [
        "provider_scope_evidence_file",
        "provider_scope_evidence_sha256",
    ]) ||
        !isDirectEphemeralPath(stringField(artifacts, "provider_scope_evidence_file")) ||
        !stringField(artifacts, "provider_scope_evidence_file").endsWith("-provider-scope-evidence.json") ||
...
async function assertWritableProviderScopeEvidenceTarget(file) {
    validateDirectChildPath("provider scope evidence", file);
    validateSuffix("provider scope evidence", file, "-provider-scope-evidence.json");
    await assertEphemeralDirectory();
    ...
    if (target !== null && !target.isFile()) {
        fail(`provider scope evidence path exists but is not a regular file: ${file}`);
    }
}
...
async function expectedProviderScopeCapturePath(headSha) {
    const rawBranch = (await git(["rev-parse", "--abbrev-ref", "HEAD"])).trim();
    const branchSlug = rawBranch === "HEAD" ? "detached" : slugBranchForFindings(rawBranch);
...
async function readSingleJsonObject(file, failureMessage) {
    try {
        const parsed = JSON.parse(await readUtf8FileStrict(file));
        if (parsed === null ||
...
async function providerBoundChangedFiles(range) {
    return parseGitChangedFilesZ(await canonicalGitDiffMetadataRaw(["--name-only", "-z", range])).sort();
}
...
async function requireRangeExistsForSurface(surface, range) {
