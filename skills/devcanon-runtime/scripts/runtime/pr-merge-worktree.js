import { CleanupUsageError, canonicalPath, collectWorktrees, currentBranch, git, isInsideWorktree, lineOutput, normalizeRemoteUrl, requireCanonicalDirectory, revParse, showRefExists, showTopLevel, validateBranchName, validateSha, worktreeStatus, } from "./cleanup-git.js";
export async function runPrMergeWorktreeCommand(args) {
    ...
    try {
        switch (subcommand) {
            ...
            default:
                throw new CleanupUsageError("usage: pr-merge-worktree preflight|cleanup");
        }
    }
    catch (err) {
...
export function reducePreflight(input) {
    ...
    if (input.outsideWorktree) {
        return stop(base, "outside-worktree", "Current directory is outside a Git worktree; re-run from a repository worktree.");
    }
    if (!input.metadataValid) {
        return stop(base, "missing-pr-metadata", input.metadataReason ??
            "Missing or unsafe PR metadata; re-run after collecting PR branch metadata.");
    }
    if (input.currentWorktree.length === 0) {
        return stop(base, "unclassifiable", "Unable to canonicalize the current worktree; re-run from a normal repository path.");
    }
    if (input.primaryWorktree.length === 0) {
        return stop(base, "missing-primary", "Unable to determine the primary worktree; run git worktree list and retry from a valid checkout.");
    }
    if (!input.knownCurrent) {
        return stop(base, "unclassifiable", "Current checkout is not present in git worktree metadata; run git worktree repair or retry from a known worktree.");
    }
    if (input.currentDetached) {
        return stop(base, "detached-current", "Current worktree is detached; re-run from a named branch worktree.");
    }
    if (input.headWorktree.length > 0 &&
        input.currentWorktree === input.headWorktree) {
        return result(base, "remote-only", "current-head-worktree", "Current worktree holds the PR head branch; merge without delegated branch cleanup, then run explicit cleanup.");
    }
    if (input.baseWorktree.length > 0 &&
        input.currentWorktree === input.baseWorktree) {
        if (input.headWorktree.length > 0 &&
            input.headWorktree !== input.currentWorktree) {
            return result(base, "remote-only", "base-with-head-worktree", "Base worktree is current and another worktree holds the PR head; avoid GitHub CLI local cleanup.");
        }
        return result(base, "safe-direct", "base-no-head-worktree", "Current worktree holds the base branch and no local worktree holds the PR head branch.");
    }
    if (input.primaryWorktree.length > 0 &&
        input.primaryWorktree === input.baseWorktree &&
        input.headWorktree.length === 0) {
        return result(base, "cd-primary", "unrelated-cd-primary", "Current worktree is unrelated; change to the primary base worktree before merge.");
    }
    return result(base, "remote-only", "unrelated-remote-only", "Current worktree is unrelated to the PR head/base collision state; use merge-only behavior and explicit cleanup.");
}
export function reduceCleanup(input) {
    ...
    if (input.headWorktreeReal.length === 0) {
        ...
        if (input.primaryWorktreeDirty) {
            ...
            manual(`inspect dirty primary worktree manually: ${input.primaryWorktreeReal}`);
        }
        else {
    ...
    if (input.prHeadBranch === input.prBaseBranch ||
        ...
        manual(`preserve local branch until head worktree cleanup is resolved: ${input.prHeadBranch}`);
    }
        ...
        manual(`delete local branch manually after inspection: ${input.prHeadBranch}`);
    }
    else {
    ...
    if (input.prHeadRepo !== input.prBaseRepo) {
    ...
    else if (input.prHeadBranch === input.prBaseBranch ||
        input.prHeadBranch === input.prBaseDefaultBranch) {
        report.remoteBranchCleanup = "retained";
        ...
        manual(`delete remote branch manually from verified base repository: ${input.prHeadBranch}`);
    }
    else if (input.remoteLookupFailed) {
        report.remoteBranchCleanup = "failed";
        report.remoteBranchCleanupReason = "git-ls-remote-failed";
        manual(`inspect remote branch after origin lookup succeeds: ${input.prHeadBranch}`);
    }
        ...
        manual(`delete remote branch manually after inspection: ${input.prHeadBranch}`);
    }
    else {
...
async function preflight() {
    ...
    const headWorktreeReal = activeWorktrees.find((worktree) => worktree.branch === headBranch)
        ?.realPath ?? "";
    const baseWorktreeReal = activeWorktrees.find((worktree) => worktree.branch === baseBranch)
        ?.realPath ?? "";
    const knownCurrent = activeWorktrees.some((worktree) => worktree.realPath === currentWorktreeReal);
    return reducePreflight({
...
async function validatePreflightMetadata(headBranch, baseBranch) {
...
async function cleanup(env) {
    ...
    const primaryWorktreeReal = await requireCanonicalDirectory(env.primaryWorktree, "PRIMARY_WORKTREE");
    const headWorktreeReal = env.headWorktree.length > 0
    ...
    const headWorktreeLockedReason = headWorktreeReal.length > 0
        ? await lockedReason(primaryWorktreeReal, headWorktreeReal)
        : null;
    const primaryWorktreeDirty = (await worktreeStatus(primaryWorktreeReal)).length > 0;
    const headWorktreeDirty = headWorktreeReal.length > 0 && headWorktreeReal !== primaryWorktreeReal
    ...
    if (env.prState === "MERGED" &&
        ...
        !headWorktreeDirty) {
        const removeResult = await git(["worktree", "remove", headWorktreeReal], primaryWorktreeReal, [0, 1, 2, 128, 255]);
    ...
    if (env.prState === "MERGED" &&
        ...
        const checkout = await git(["checkout", env.prBaseBranch], primaryWorktreeReal, [0, 1]);
        const pull = checkout.exitCode === 0
            ? await git(["pull", "--ff-only"], primaryWorktreeReal, [0, 1])
            : { exitCode: 1 };
        baseUpdateFailed = checkout.exitCode !== 0 || pull.exitCode !== 0;
    }
    const localBranchExists = await showRefExists(primaryWorktreeReal, `refs/heads/${env.prHeadBranch}`);
    const localTip = localBranchExists
        ? await revParse(primaryWorktreeReal, `refs/heads/${env.prHeadBranch}`)
        : "";
    const branchStillCheckedOut = await branchCheckedOut(primaryWorktreeReal, env.prHeadBranch);
    const canDeleteLocal = env.prState === "MERGED" &&
    ...
    if (canDeleteLocal) {
        localDeleteFailed =
            (await git(["branch", "-D", env.prHeadBranch], primaryWorktreeReal, [0, 1])).exitCode !== 0;
    }
    ...
    if (env.prState === "MERGED" &&
        ...
        env.prHeadBranch !== env.prBaseBranch &&
        env.prHeadBranch !== env.prBaseDefaultBranch) {
        const originUrl = (await git(["remote", "get-url", "origin"], primaryWorktreeReal, [0, 1])).stdout.trim();
        originMatchesBase =
            originUrl.length > 0 &&
                (await normalizeRemoteUrl(originUrl)) ===
                    (await normalizeRemoteUrl(env.prBaseRemoteUrl));
        if (originMatchesBase) {
            const listing = await git(["ls-remote", "--heads", "origin"], primaryWorktreeReal, [0, 1, 2, 128]);
            ...
            if (!remoteLookupFailed) {
                ...
                if (remoteSha === env.prHeadSha) {
                    remoteDeleteFailed =
                        (await git(["push", "origin", `:refs/heads/${env.prHeadBranch}`], primaryWorktreeReal, [0, 1])).exitCode !== 0;
                }
            }
        }
    }
...
async function branchCheckedOut(primaryWorktree, branch) {
    return (await collectWorktrees(primaryWorktree)).some((worktree) => worktree.branch === branch);
}
async function lockedReason(primaryWorktree, targetReal) {
...
function requireEnv(name) {
    ...
    if (value === undefined || value.length === 0) {
        throw new CleanupUsageError(`Missing required environment variable: ${name}`);
    }
    if (name.endsWith("BRANCH") && !validateBranchNameSync(value)) {
        throw new CleanupUsageError(`Invalid ${name}: ${value}`);
    }
    if (name === "PR_HEAD_SHA" && !validateSha(value)) {
        throw new CleanupUsageError("PR_HEAD_SHA must be a 40-character lowercase hex SHA");
    }
    return value;
}
function validateBranchNameSync(value) {
...
function stop(base, reasonCode, reason) {
    return result(base, "stop", reasonCode, reason);
}
function result(base, mode, reasonCode, reason) {
