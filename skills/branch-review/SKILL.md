...
| `--risk-signals <repo-relative-path>` | Optional, non-authoritative repo-relative `.ephemeral/*-risk-signals.json` handoff from planning or execution. Valid signals can only preserve or escalate scrutiny; invalid supplied signals fail closed.                                          |
...
if [ -n "${RISK_SIGNALS_SEMANTIC_ESCALATION_REASON:-}" ]; then
...
SEMANTIC_DECISION_NOTES="${RISK_SIGNALS_SEMANTIC_DECISION_NOTES:-}" \
