...
`capture` and `verify` require the issue worktree's real Git root and `.ephemeral`; `cleanup` uses its physical current directory as the cleanup root and accepts no Git-worktree requirement.
