...
digraph priming {
  ...
  immediate_internal_research [label="3a. Root dispatches exactly one required internal research-agent\nimmediate path: policy + ADR + code + tests"];
  ...
  late_internal_research [label="3a. Root dispatches exactly one required internal research-agent\nlate path: wait for policy + ADR + code + tests report"];
