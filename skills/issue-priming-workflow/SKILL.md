...
- **Judgment-required nits** — leave unfixed. After classification, write the judgment-required subset as a fresh `play-review/findings/v1` envelope to `.ephemeral/<branch_slug>-<head_sha>-nits-pending.json` (compute `<branch_slug>` and `<head_sha>` the same way `play-review` does — see `skills/play-review/SKILL.md` § Output). The Phase 8 step "Pass nits to `play-branch-finish`" passes that file path as `nits_file`. If the judgment-required set is empty, skip the file write — Phase 8 will omit `nits_file` entirely.
...
**Pass nits to `play-branch-finish`:** When invoking `play-branch-finish` for PR creation, pass `nits_file` — a repo-relative path to a `play-review/findings/v1` envelope (schema in `skills/play-review/SKILL.md` § Output; transport per [ADR-0012](../../docs/adr/adr-0012-side-channel-file-delivery-for-play-review-findings.md)) containing only the judgment-required subset. Build the file in Phase 7 as follows: after the per-nit classification, take every `severity: "Nit"` finding tagged judgment-required, wrap them in a fresh envelope `{"schema": "play-review/findings/v1", "findings": [<judgment-required items>], "carry_forward": []}`, and write to `.ephemeral/<branch_slug>-<head_sha>-nits-pending.json` (the `-nits-pending` suffix distinguishes this derived file from the `-findings.json` file `play-review` wrote). `play-branch-finish` posts every entry of `findings[]` in that file — anchorable items as inline review comments, unanchorable as a top-level review comment — applying `"side": "RIGHT"` defaults and dropping `start_line: null`. Only items with `severity: "Nit"` are eligible; any `severity: "Blocking"` items must already have triggered a stop-`--auto` in Phase 7, so they never reach this file. Extra fields (`severity`, `category`, `critic`, `anchor`, `why`, `recommendation`) are ignored by `play-branch-finish` but harmless to leave in the envelope. If there are no judgment-required nits, omit the `nits_file` arg entirely; `play-branch-finish` handles the absence by skipping the post step.
