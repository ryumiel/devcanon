...
| `incomplete_topical_routes` | array                                                                                          | Selected D7-D9 routes that returned `NEEDS_CONTEXT`, `FAILED`, or reached a controller-observed orchestration failure. Each entry has `route` (`D7`, `D8`, or `D9`) and `disposition` (`NEEDS_CONTEXT`, `FAILED`, or `CONTROLLER_OBSERVED_FAILURE`). This is approval evidence, not a finding or critic input. |
