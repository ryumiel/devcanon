...
Runs the planning source-immutability lifecycle adapter.

## Invocation

Run `capture [--handoff .ephemeral/<file>]`, `verify --baseline .ephemeral/.devcanon-source-immutability-<hex>.json [--handoff .ephemeral/<file>]`, or `cleanup --baseline .ephemeral/.devcanon-source-immutability-<hex>.json [--handoff .ephemeral/<file>]` through `bash "$PLAY_PLANNING_DIR/scripts/source-immutability.sh"`.
...
`capture` and `verify` require the planning worktree's real Git root and `.ephemeral`; `cleanup` uses its physical current directory as the cleanup root and accepts no Git-worktree requirement.
...
[Play planning workflow context](../SKILL.md) owns disposition and continuation.
