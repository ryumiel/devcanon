...
implementation work when the design changes durable truth. The canonical
trigger list lives in `docs/guidelines/documentation-standard.md` §5.2; common
examples include interfaces or schemas, major paths or layout, behavior,
workflow, commands, ownership, verification, architecture, and policy. If the
input design has a "Documentation impact" section, every listed file must have a
corresponding task in the plan. New ADRs use `docs/adr/adr-template.md` as the
source. For routing boundaries, follow
