...
**4. Example verification:** For any worked example, code snippet annotation, or scenario reference in the plan that names a specific file path, line number, function name, identifier, command, commit SHA, or PR number — open the file (or run `git log` / `gh pr view`) and confirm the cited artifact exists and contains the cited text. A scenario explicitly labeled `(hypothetical)` is exempt. A scenario labeled "from PR #N" or citing a real file path is **not** exempt — verify it. Concrete-looking specifics that turn out to be fabricated are the most common silent defect class in worked examples. Note: this complements the existing Plan Review subagent check on task file paths — the subagent verifies file paths in task headers; this self-review item verifies citations in worked examples and prose.
