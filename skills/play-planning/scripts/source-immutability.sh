...
if [ "${1:-}" = "--help" ]; then
  ...
  usage_document="$script_dir/../references/source-immutability-usage.md"
...
[ -x "$runtime_resolver" ] ||
  fail "devcanon-runtime passive runtime bundle missing for source immutability"

runtime_entrypoint="$("$runtime_resolver" resolve-entrypoint --from "$script_path" --entrypoint "scripts/devcanon-runtime.sh")"
exec "$runtime_entrypoint" runtime source-immutability "$@"
