...
Fetch a GitHub issue, set up an isolated worktree, assess whether it needs multi-agent research, and invoke brainstorming with pre-loaded context. The research phase runs in a dedicated agent to keep the main session clean.
...
If the issue cannot be fetched (not found, `gh` not authenticated), stop and report the error.
