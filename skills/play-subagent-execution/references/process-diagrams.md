...
digraph process {
    ...
    "Proof VERIFIED and HEAD unchanged?" [shape=diamond];
    ...
    "Rerun affected task review and read-only proof" [shape=box];
    ...
    "Task execution route?" -> "Run guarded inline or assessor read-only proof" [label="read-only proof"];
    "Run guarded inline or assessor read-only proof" -> "Proof VERIFIED and HEAD unchanged?";
    "Proof VERIFIED and HEAD unchanged?" -> "Mark task complete" [label="yes"];
    "Proof VERIFIED and HEAD unchanged?" -> "Blocking result owner?" [label="proof failed; guards clean"];
    "Proof VERIFIED and HEAD unchanged?" -> "Stop: BLOCKED for guard integrity" [label="guard failure"];
    ...
    "D12 fixes implementation finding" -> "Rerun affected task review and read-only proof";
    "Rerun affected task review and read-only proof" -> "Fresh D16 capture";
