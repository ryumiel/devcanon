...
digraph process {
    ...
    "Task execution route?" [shape=diamond];
    "All implementation members complete for assigned entries?" [shape=diamond];
    "Run guarded inline or assessor read-only proof" [shape=box];
    "All assigned boundaries VERIFIED at current HEAD?" [shape=diamond];
    ...
    "Verified auto one-mutating-task D16 skip: no non-diff proof and branch-review guaranteed?" [shape=diamond];
    ...
    "Blocking result owner?" [shape=diamond];
    "D12 fixes implementation finding" [shape=box];
    "Rerun affected task review and proof assignments" [shape=box];
    "Return reviewed-plan defect to planning and rerun affected execution" [shape=box];
    ...
    "Stop: BLOCKED for guard integrity" [shape=box];
    ...
    "Task contract structurally valid?" -> "Task execution route?" [label="yes"];
    "Task execution route?" -> "All implementation members complete for assigned entries?" [label="read-only proof"];
    "All implementation members complete for assigned entries?" -> "Run guarded inline or assessor read-only proof" [label="yes"];
    "All implementation members complete for assigned entries?" -> "Stop: BLOCKED/NEEDS_CONTEXT for task contract" [label="no"];
    "Run guarded inline or assessor read-only proof" -> "All assigned boundaries VERIFIED at current HEAD?";
    "All assigned boundaries VERIFIED at current HEAD?" -> "Mark task complete" [label="yes"];
    "All assigned boundaries VERIFIED at current HEAD?" -> "Blocking result owner?" [label="proof failed; guards clean"];
    "All assigned boundaries VERIFIED at current HEAD?" -> "Stop: BLOCKED for guard integrity" [label="guard failure"];
    "Task execution route?" -> "Plan has exactly one task?" [label="source-mutating"];
    ...
    "More tasks remain?" -> "Task execution route?" [label="yes"];
    "More tasks remain?" -> "Verified auto one-mutating-task D16 skip: no non-diff proof and branch-review guaranteed?" [label="no"];
    "Verified auto one-mutating-task D16 skip: no non-diff proof and branch-review guaranteed?" -> "Return to caller" [label="yes"];
    "Verified auto one-mutating-task D16 skip: no non-diff proof and branch-review guaranteed?" -> "Fresh D16 capture" [label="no"];
    ...
    "Final whole-implementation review passes?" -> "Blocking result owner?" [label="no"];
    "Blocking result owner?" -> "D12 fixes implementation finding" [label="authorized implementation"];
    "Blocking result owner?" -> "Return reviewed-plan defect to planning and rerun affected execution" [label="reviewed plan"];
    "D12 fixes implementation finding" -> "Rerun affected task review and proof assignments";
    "Rerun affected task review and proof assignments" -> "Fresh D16 capture";
    "Return reviewed-plan defect to planning and rerun affected execution" -> "Fresh D16 capture";
