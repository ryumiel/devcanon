  ...
  prompt: |
    ...
    **Task contract checklist (when present in the requested task):**
    ...
    - In the case when task text includes Contract Example Discipline or an
      equivalent clearly labeled section/obligation, independently verify the
      source-backed example consistency obligations as spec-compliance checks.
      ...
      dimension unless multi-fault behavior is intentional and named; and
      derived fields stay consistent with source facts or are explicitly
      justified. Do not decide whether Contract Example Discipline should have
      been required; `play-planning` owns that trigger taxonomy. Report exact
      source-backed gaps instead of accepting or guessing through inconsistent
      examples.
