...
owns this invocation and, after Candidate Closure and Source Freeze, Phase 7
runs `branch-review --fix` on the full branch diff. Any source mutation returns
through candidate closure before the next review. The final run reports zero
blocking findings auto-fixed, no unresolved remaining `Blocking` findings
except findings whose `critic` verdict is `INVALID` or `DOWNGRADE`, has a
captured final approval-summary notice path, and provides fresh final
