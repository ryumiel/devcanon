...
response. Reviewers produce evidence; the controller owns disposition under
`lifecycle-status-policy.md`. Do not classify findings for mutation or infer a
D12 route from severity, validity, or technical fixability. Never reuse a D15
session or result as D16.
