...
| 3   | No clarifying questions could plausibly arise | Implicit: `play-planning` plan-review returned PASS upstream. For direct invocations with no upstream PASS, treat this guardrail as PASS and rely on the remaining four.                                          |
| 4   | Task contract gate is satisfied               | The task has either a structurally complete `**Contract checklist:**` naming every required checklist surface, or a task-specific no-trigger omission reason backed by upstream `play-planning` plan-review PASS. |
