...
3. If `owner_thread_id` is missing, first reconcile any pending owner creation
   ...
   dispatch tuple and current effect authority; preflight that the host supports
   both task creation and its existing identity operation for confirming a
   top-level owner's ID, depth, and host identity when task IDs are scoped;
   discover a compatible existing **top-level owner task** using that key; and,
   only when no compatible owner exists, retain an in-flight attempt before
   creating one separate top-level owner task with the source-specific priming
   prompt as its initial work. Missing or unknown required identity capability
   waits or reports before creation; do not invent an identity from a checkout.
...
5. Integrate any owner-thread gate report or
...
The following fourteen bounded fixture families are the self-check surface for the
