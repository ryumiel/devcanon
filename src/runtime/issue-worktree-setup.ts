...
async function runIssueWorktreeSetup(
  ...
  if (baseRef === undefined) {
    ...
    if (defaultBranchResult.exitCode !== 0) {
      return plainFail(
        `Unable to determine origin's default branch: ${defaultBranchResult.stderr.trim() || "git ls-remote --symref --exit-code origin HEAD failed"}`,
      );
    }
    ...
    if (symbolicHeadTargets.length === 0) {
      return plainFail(
        "Unable to determine origin's default branch: origin did not advertise a symbolic HEAD target",
      );
    }
    if (symbolicHeadTargets.length !== 1) {
      return plainFail(
        "Unable to determine origin's default branch: origin advertised multiple symbolic HEAD targets",
      );
    }
    ...
    if (
      !symbolicHeadTarget.startsWith(branchPrefix) ||
      symbolicHeadTarget.length === branchPrefix.length
    ) {
      return plainFail(
        `Unable to determine origin's default branch: origin advertised an invalid symbolic HEAD target: ${symbolicHeadTarget}`,
      );
    }
