...
export async function runResolveBashCommand(
...
async function resolveGitForWindowsBash(
  env: NodeJS.ProcessEnv,
): Promise<string> {
  const candidates = new Set<string>();
  if (env.DEVCANON_GIT_BASH) candidates.add(env.DEVCANON_GIT_BASH);
  ...
  for (const candidate of candidates) {
    if (isRejectedWindowsLauncher(candidate)) continue;
    try {
...
async function resolvePosixBash(env: NodeJS.ProcessEnv): Promise<string> {
...
export function isRejectedWindowsLauncher(candidate: string): boolean {
  ...
  return (
    ...
    basename === "wsl.exe" ||
    (basename === "bash.exe" &&
      ["system32", "sysnative", "syswow64"].includes(
        path.win32.basename(directory),
      ))
  );
}
