...
export async function runIssuePrimingCommand(
  ...
  try {
    ...
    switch (command) {
      ...
      default:
        return plainFail(
          `unknown issue-priming runtime command: ${command ?? "<missing>"}`,
        );
    }
  } catch (err) {
...
async function validatePhaseArtifact(
  args: readonly string[],
): Promise<RuntimeCommandOutcome> {
  if (args.length !== 3 || args[0] !== "validate-read") {
    return plainFail(
      "usage: phase-artifacts validate-read <kind> <repo-relative-path>",
    );
  }
  await requireRepositoryRoot("phase-artifacts");
  const kind = args[1] as keyof typeof ARTIFACT_KINDS;
...
async function prepareResearchBrief(
  ...
  if (slug.length === 0) {
    return plainFail(
      "ISSUE_IDENTIFIER must contain at least one slug character",
    );
  }
...
async function writeAutoHandoff(
  ...
  if (!SHA_PATTERN.test(headSha)) {
    return plainFail(
      "git rev-parse HEAD did not return a 40-character lowercase hex SHA",
    );
  }
  ...
  await writeTextAtomically(
    path.join(cwd, ...target.split("/")),
    `${JSON.stringify(payload, null, 2)}\n`,
  );
  return plainOk(`${target}\n`);
}

async function prepareAssumptionsComment(
  args: readonly string[],
  env: NodeJS.ProcessEnv,
): Promise<RuntimeCommandOutcome> {
...
async function requireRepositoryRoot(helper: string): Promise<string> {
...
function requireOwnedPath(
  ...
  let directChild: ReturnType<typeof requireDirectEphemeralChild>;
  try {
    ...
    if (message.includes("direct child")) {
      if (label === "assumptions_comment_file") {
        throw new Error(
          `${label} must be a direct child of .ephemeral: ${candidate}`,
        );
      }
...
async function assertReadableRegularFile(
  ...
  try {
    ...
    throw new Error(
      `${label} missing or not a regular file: ${repoRelativePath}`,
    );
  }
...
async function prepareWriteTarget(
  repoRelativePath: string,
  label: string,
): Promise<void> {
  ...
  try {
    ...
    if (!stat.isFile()) {
      throw new Error(
        `${label} path exists but is not a regular file: ${repoRelativePath}`,
      );
    }
  } catch (err) {
...
async function assertSafeEphemeralDirectory(create: boolean): Promise<void> {
...
function slugIdentifier(identifier: string): string {
  return identifier
    .toLowerCase()
    .replaceAll("/", "-")
    .replace(/[^a-z0-9._-]/gu, "");
