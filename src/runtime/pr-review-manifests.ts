...
export async function runPrReviewManifestsCommand(
...
async function prepareHandoffWrite(): Promise<string> {
...
async function prepareResultWrite(): Promise<string> {
  await requireRepoRoot();
  const prNumber = readPrNumber();
  const headSha = readHeadSha();
...
async function writeHandoff(): Promise<string> {
  await requireRepoRoot();
  const prNumber = readPrNumber();
  const headSha = readHeadSha();
  ...
  const executionWorkingDirectory = normalizeExecutionWorkingDirectory(
    requiredEnv("EXECUTION_WORKING_DIRECTORY"),
  );
...
async function writeResult(): Promise<string> {
  ...
  const result = {
    ...
    digests: {
      ...
      review_body_sha256:
        reviewBodyFile === null ? null : await sha256File(reviewBodyFile),
      context_sha256:
        contextFile === null ? null : await sha256File(contextFile),
...
async function renderPhase5AuditSummary(): Promise<string> {
  ...
  const resultFile = requiredEnv("RESULT_FILE");
  validateDirectChildPath("result", resultFile, "-result.json");
  const primaryRoot = await requireAbsoluteDirectory(
    "PRIMARY_REPOSITORY_ROOT",
    requiredEnv("PRIMARY_REPOSITORY_ROOT"),
  );
  const worktreeRoot = await requireAbsoluteDirectory(
    "WORKTREE_PATH",
  ...
  return [
    ...
    `- Finding counts: ${code(String(findingItems.length))} active, ${code(String(carryForwardItems.length))} carry-forward`,
...
async function readLeaseStatus(
  ...
  const currentResultSha256 = await withCwd(worktreeRoot, () =>
    sha256File(resultFile),
  );
...
function parseLeaseStatus(stdout: string): LeaseStatus {
  let parsed: unknown;
  try {
  ...
  const status = {
    lease_state: stringField(
      parsed,
      "lease_state",
      "read-status schema mismatch",
    ),
    ...
    worktree_digest: stringField(
      parsed,
      "worktree_digest",
      "read-status schema mismatch",
    ),
    worktree_exists: booleanField(
      parsed,
      "worktree_exists",
      "read-status schema mismatch",
    ),
    worktree_registered: booleanField(
      parsed,
      "worktree_registered",
      "read-status schema mismatch",
    ),
    worktree_dirty: booleanField(
      parsed,
      "worktree_dirty",
      "read-status schema mismatch",
    ),
    identity_match: booleanField(
      parsed,
      "identity_match",
      "read-status schema mismatch",
    ),
    ...
    result_sha256: stringField(
      parsed,
      "result_sha256",
      "read-status schema mismatch",
    ),
    result_validated_at: stringField(
      parsed,
      "result_validated_at",
      "read-status schema mismatch",
    ),
    lease_updated_at: stringField(
      parsed,
      "lease_updated_at",
      "read-status schema mismatch",
    ),
    presentation_status: stringField(
      parsed,
      "presentation_status",
      "read-status schema mismatch",
    ),
    presented_at: stringField(
      parsed,
      "presented_at",
      "read-status schema mismatch",
    ),
  ...
  if (
    !isTimestamp(status.result_validated_at) ||
  ...
  if (
    status.presentation_status !== "preview-current" &&
...
async function requireAbsoluteDirectory(
  label: string,
  value: string,
): Promise<string> {
...
async function validateHandoffFacts(handoff: JsonObject, identityFile: string) {
  const prNumber = readPrNumber();
  const headSha = readHeadSha();
...
async function validateResultFacts(result: JsonObject, identityFile: string) {
  const prNumber = String(readPrNumber());
  const headSha = readHeadSha();
  const manifestPrNumber = String(numberField(result, "pr_number"));
  if (manifestPrNumber !== prNumber) {
    fail(
      `result PR number mismatch: manifest ${manifestPrNumber}, current ${prNumber}`,
    );
  }
...
async function validateFindingsAuthority(findingsFile: string) {
...
async function resolvePrReviewDir(): Promise<string> {
...
async function resolvePlayReviewHelper(): Promise<string> {
...
async function validateOptionalDirectChildReadableArtifact(
  label: string,
  file: string | null,
  suffix: string,
) {
...
async function assertReadableFile(label: string, file: string) {
  await guardEphemeral();
...
async function prepareWriteTarget(label: string, file: string) {
  await guardEphemeral();
  await mkdir(path.join(process.cwd(), ".ephemeral"), { recursive: true });
...
async function guardEphemeral() {
  const stat = await lstat(path.join(process.cwd(), ".ephemeral")).catch(
    () => null,
  );
  if (stat?.isSymbolicLink()) {
...
function validateShaValue(label: string, value: string) {
  if (!isSha(value)) {
...
function parseJsonEnv(name: string): unknown {
  try {
    return JSON.parse(requiredEnv(name)) as unknown;
  } catch {
