...
export const PR_REVIEW_GOVERNED_PATH_PATTERN =
  "^(docs/(adr|arch|product-requirements|specs|guidelines)/|MAP\\.md$|AGENTS\\.md$|CONTRIBUTING\\.md$)";
...
const reviewArtifactRootContext = new AsyncLocalStorage<string>();

function reviewArtifactRoot(): string {
  return reviewArtifactRootContext.getStore() ?? process.cwd();
}

function reviewArtifactPath(file: string): string {
...
async function validateScopeDecision(
  ...
  if (lastReviewed.length > 0) {
    ...
    const derivedMechanicalScopeCodes = new Set<string>();
...
async function validatePrReviewProviderEvidence(
  ...
  const expectedEvidenceFile = await expectedProviderScopeEvidencePath(
    options.headSha,
  );
  ...
  const evidenceText = await readFile(
    reviewArtifactPath(options.providerScopeEvidenceFile),
    "utf-8",
  );
  ...
  if (
    stringField(digestProvenance, "local_diff") !== CANONICAL_GIT_DIFF_DIALECT
  ) {
...
function digestProvenanceField(evidence: JsonObject): JsonObject {
  return objectField(evidence, "digest_provenance");
}

function hasCompatibleFullDiffProvenance(provenance: JsonObject): boolean {
  return (
    stringField(provenance, "provider_diff") === CANONICAL_GIT_DIFF_DIALECT &&
    stringField(provenance, "local_diff") === CANONICAL_GIT_DIFF_DIALECT
  );
}

function validatePatchDigestProvenance(provenance: JsonObject): void {
  if (
...
function isDigestProvenance(value: unknown): value is JsonObject {
...
function canonicalGitConfigArgs(
  orderFile: string,
...
async function assertEmptyOrMissingInfoAttributes(
  ...
  return reviewArtifactRootContext.run(input.worktreeRoot, async () => {
    ...
    const expectedPayload = buildApprovedReviewPayload({
      ...
      reviewBody: await readFile(
        reviewArtifactPath(options.reviewBodyFile),
        "utf-8",
      ),
      findings,
    });
...
export async function validatePrReviewEvidenceAuthority(input: {
  ...
  await reviewArtifactRootContext.run(input.worktreeRoot, async () => {
    const priorContextKind =
      input.priorThreadsFile === null ? "none" : "github-prior-threads";
    const priorContextPath = input.priorThreadsFile ?? "null";
    const scopeOptions: ReviewArtifactOptions = {
      ...EMPTY_OPTIONS,
      surface: "pr-review",
...
function branchScopeReasonCodes(scope: JsonObject): string[] {
  if (!Array.isArray(scope.scope_reason_codes)) {
    fail("scope_reason_codes must be an array");
  }
  if (
    !scope.scope_reason_codes.every((code) => typeof code === "string") ||
    !stringArrayField(scope, "scope_reason_codes").every(
      (code) => code.length > 0,
    )
  ) {
...
function validateBranchScopeReasonConsistency(
  scopeReasonCodes: string[],
  facts: {
    isNarrow: boolean;
    isInitial: boolean;
    escalationReasons: readonly string[];
    semanticAmbiguous: boolean;
    expectedMechanicalCodes: ReadonlySet<string>;
  },
): void {
  const actualCodes = new Set(scopeReasonCodes);
  if (facts.isNarrow) {
    if (
      scopeReasonCodes.length !== 1 ||
...
async function assertEphemeralDirectory(): Promise<void> {
  const stat = await lstat(reviewArtifactPath(".ephemeral")).catch(
    () => undefined,
  );
  if (stat?.isSymbolicLink()) {
...
async function validateFindingsPathMatchesHead(
  findingsFile: string,
  headSha: string,
): Promise<void> {
...
async function expectedProviderScopeEvidencePath(
  headSha: string,
): Promise<string> {
  const rawBranch = (await git(["rev-parse", "--abbrev-ref", "HEAD"])).trim();
  const branchSlug =
