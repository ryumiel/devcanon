import { CleanupUsageError, collectWorktrees, countStatusLines, git, isBareRepository, isInsideWorktree, lineOutput, localBranches, resolveDefaultBranch, revParse, showRefExists, showTopLevel, worktreeStatus, } from "./cleanup-git.js";
const usage = "usage: git-workspace-cleanup.sh [--repo <path>] [--dry-run|--execute] [--force-branches] [--force-dirty-worktrees]";
class HelpRequested extends Error {
    constructor() {
        super(usage);
    }
}
export async function runGitWorkspaceCleanupCommand(args) {
    try {
...
export function reduceWorkspaceStatus(input) {
    if (input.dirtyPrimaryCount > 0)
        return "blocked";
    if (input.defaultBranchAheadCommits > 0)
        return "blocked";
    if (input.dirtyLinkedCount > 0 &&
        (input.mode === "dry-run" || !input.forceDirtyWorktrees)) {
        return "blocked";
    }
    if (input.lockedWorktreeCount > 0)
        return "blocked";
    if (input.uniqueBranchCount > 0 &&
...
async function collectFacts(parsed) {
    if (!(await isInsideWorktree(parsed.targetRepo))) {
        throw new CleanupUsageError(`not inside a git worktree: ${parsed.targetRepo}`);
    }
    ...
    const prunableWorktrees = allWorktrees.filter((worktree) => worktree.prunable);
    ...
    const branches = await collectBranches(repoRoot, defaultBranch, remoteDefaultRef);
    const defaultBranchAheadCommits = await defaultAheadCount(repoRoot, defaultBranch, remoteDefaultRef);
    const dirtyPrimaryCount = dirtyWorktrees.filter((entry) => entry.primary).length;
    ...
    const uniqueBranchCount = branches.filter((branch) => branch.classification === "unique").length;
...
async function collectBranches(repoRoot, defaultBranch, remoteDefaultRef) {
    const facts = [];
    for (const branchRef of await localBranches(repoRoot)) {
        ...
        if ((await git(["merge-base", "--is-ancestor", branchRef, remoteDefaultRef], repoRoot, [0, 1])).exitCode === 0) {
            facts.push({
                branch,
                ref: branchRef,
        ...
        else if (await branchIsSquashMerged(repoRoot, branchRef, remoteDefaultRef)) {
            facts.push({
                branch,
                ref: branchRef,
            ...
            const commits = Number((await git(["rev-list", "--count", `${remoteDefaultRef}..${branchRef}`], repoRoot, [0, 1])).stdout.trim() || "1");
            facts.push({
                branch,
                ref: branchRef,
...
async function branchIsSquashMerged(repoRoot, branchRef, remoteDefaultRef) {
    const mergeBase = (await git(["merge-base", remoteDefaultRef, branchRef], repoRoot, [0, 1])).stdout.trim();
    if (mergeBase.length === 0)
        return false;
    const tree = (await git(["rev-parse", `${branchRef}^{tree}`], repoRoot, [0, 1])).stdout.trim();
    if (tree.length === 0)
        return false;
    const dummyCommit = (await git(["commit-tree", tree, "-p", mergeBase, "-m", "_"], repoRoot, [0, 1])).stdout.trim();
    if (dummyCommit.length === 0)
        return false;
    const cherry = (await git(["cherry", remoteDefaultRef, dummyCommit], repoRoot, [0, 1])).stdout.trim();
    return cherry.startsWith("-");
}
async function defaultAheadCount(repoRoot, defaultBranch, remoteDefaultRef) {
    ...
    const value = (await git(["rev-list", "--count", `${remoteDefaultRef}..${defaultRef}`], repoRoot, [0, 1])).stdout.trim();
    const parsed = Number(value || "0");
    return Number.isFinite(parsed) ? parsed : 0;
}
function formatReport(facts) {
    const mergedBranches = facts.branches.filter((branch) => branch.classification !== "unique");
    const uniqueBranches = facts.branches.filter((branch) => branch.classification === "unique");
    const lines = [
        ["MODE", facts.mode],
        ["STATUS", facts.status],
...
async function executeCleanup(parsed, facts) {
    for (const worktree of facts.worktrees.slice(1)) {
        const status = await worktreeStatus(worktree.realPath);
        if (status.length > 0) {
            if (!parsed.forceDirtyWorktrees) {
                throw new Error(`linked worktree became dirty before removal: ${worktree.realPath}`);
            }
            await git(["worktree", "remove", "--force", worktree.realPath], facts.primaryWorktree);
        }
        else {
            await git(["worktree", "remove", worktree.realPath], facts.primaryWorktree);
        }
    }
    await git(["worktree", "prune"], facts.primaryWorktree);
    if (await showRefExists(facts.primaryWorktree, `refs/heads/${facts.defaultBranch}`)) {
        await git(["checkout", facts.defaultBranch], facts.primaryWorktree);
    }
    else {
        await git(["checkout", "-b", facts.defaultBranch, `origin/${facts.defaultBranch}`], facts.primaryWorktree);
    }
    await git(["merge", "--ff-only", `origin/${facts.defaultBranch}`], facts.primaryWorktree);
    for (const branch of facts.branches) {
